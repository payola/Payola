...
`Circle`, `Gravity` and `Tree` visualizations display a regular graph. They represent the entities as circles (identified vertices - entities with a definite URI) interconected by arrows (relations - oriented edges). The literal vertices (entities not containing URI) are contained as vertices attributes and are show in a table only if one vertex is selected. The visualizations also show vertices URI (or a label if a vertex has such an attribute) and edges URI (if one of the edge's vertices is selected). The difference between the plugins is only in the initial animated layout. Controls for handling the graph are the same for all three plugins.

These visualizations allow you to select vertices. Single vertex can be selected by pressing a mouse button over it. Multiple vertices can be selected by holding a Shift key while selecting vertices. Deselecting vertices can be done by holding Shift and pressing a mouse button over a selected vertex or simply by selecting not selected vertex without holding a Shift key. Deselecting all vertices is done by pressing mouse between vertices (not over a vertex). These visualizations also allow you to move selected vertices around. By pressing and holding down a mouse button over a vertex and moving it (dragging) moves all selected vertices of the graph. Whole graph can also be moved by dragging mouse between vertices. These visualizations provide a zoom tool. Zooming in or out can be done by rolling the wheel of your mouse or by pressing the `Zoom in` or `Zoom out` buttons in the controls bar shown on the top of the graph. Between the buttons in the controls bar is shown a current zoom status. Its default value is set to 100%. The controls bar contains also a `Stop animation` button. This button can effect the initial layout animation. If the animation takes a long time to finish, this button stops it. The animation is also stopped if the controls are used. Pressing a mouse button or zooming interrupts the animation and allows you to control the visualization. Another button specific for these three visualizations is `Download as PNG`. Pressing this button saves the image of the graph in the PNG format.

The reaction time of the controls and speed of the initial animations may be effected by hardware configuration of your computer. It also may be effected by the web browser you are using.
