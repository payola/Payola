...
import scala.collection.mutable.{ListBuffer, StringBuilder, LinkedHashSet}
...
class ScalaToJsComponent(val plugin: ScalaToJsPlugin) extends PluginComponent
{
    ...
    class ScalaToJsPhase(prev: Phase) extends StdPhase(prev)
    {
        ...
        override def apply(unit: CompilationUnit) {
            ...
            val outputFile = new File(plugin.output.getAbsolutePath + "/" + packagePath + "/" + outputFileName)
    ...
    def isDefaultThing(tree: Tree): Boolean = {
    ...
    def buildPackageLevelItem(t: Tree): String = {
        t match {
    ...
    def buildPackageLevelItemMember(t: Tree): String = {
        t match {
    ...
    def buildModule(moduleDef: ModuleDef): String = {
        ...
        def buildMembersFromTrait(t: Tree) = {
