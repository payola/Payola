...
{
    def executeQuery(query: String): PayolaGraph = {
        val queryUrl = endpointURL + "?query=" + java.net.URLEncoder.encode(query, "UTF-8")
        QueryFactory.create(query).getQueryType match {
            case Query.QueryTypeConstruct => {
                PayolaGraph(RdfRepresentation.RdfXml, new Downloader(queryUrl, "application/rdf+xml", credentials = Some((username, password))).result)
            }
            case Query.QueryTypeSelect => {
                PayolaGraph(RdfRepresentation.Turtle, new Downloader(queryUrl, "text/rdf+n3", credentials = Some((username, password))).result)
            }
