...
class AnalysisRunner(elementToDrawIn: String, analysisId: String) extends Presenter
{
    ...
    private def getSuccessEventHandler(analysis: Analysis, view: AnalysisRunnerView): (EvaluationSuccessEventArgs => Unit) = {
        evt: EvaluationSuccessEventArgs =>
            ...
            //graphPresenter.view.setEvaluationId(getAnalysisEvaluationID) //TODO disabled cache
    ...
    private def pollingHandler(view: AnalysisRunnerView, analysis: Analysis, persistInAnalysisStorage: Boolean) {
        AnalysisRunner.getEvaluationState(evaluationId, analysis.id, false /*true TODO disabled cache*/, persistInAnalysisStorage, false) {
            state =>
                state match {
