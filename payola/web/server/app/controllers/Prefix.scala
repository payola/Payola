...
object Prefix extends PayolaController with Secured
{
    def list(page: Int = 1) = maybeAuthenticated { user: Option[User] =>
        ...
        val availablePrefixes = Payola.model.prefixModel.getAllAvailableToUser(userId)
        Ok(views.html.prefix.list(user, availablePrefixes, page))
    }

    def create = authenticatedWithRequest {(user, request) =>
        Ok(views.html.prefix.list(Some(user), Nil, 1))
    }
    ...
    def detail(id: String) = authenticatedWithRequest {(user, request) =>
        Ok(views.html.prefix.list(Some(user), Nil, 1))
    }
