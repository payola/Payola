...
(function() {
    if (!Array.prototype.some)
        Array.prototype.some = function(i, m) {
            var f = this.length;
            if ("function" != typeof i)
                throw new TypeError;
            for (var j = 0; j < f; j++)
                if (j in this && i.call(m, this[j], j, this))
                    return !0;
            return !1
        };
    if (!Array.prototype.forEach)
        Array.prototype.forEach = function(i, m) {
            var f = this.length;
            if ("function" != typeof i)
                throw new TypeError;
            for (var j = 0; j < f; j++)
                j in this && i.call(m, this[j], j, this)
        };
    
    if (!Array.prototype.map)
        Array.prototype.map = function(i, m) {
            var f = this.length;
            if ("function" != typeof i)
                throw new TypeError;
            for (var j = Array(f), n = 0; n < f; n++)
                n in this && (j[n] = i.call(m, this[n], n, this));
            return j
        };
    if (!Array.prototype.filter)
        Array.prototype.filter = function(i, m) {
            var f = this.length;
            if ("function" != typeof i)
                throw new TypeError;
            for (var j = [], n = 0; n < f; n++)
    ...
    if (!Object.keys)
        Object.keys = function() {
            var i = Object.prototype.hasOwnProperty, m = !{toString: null}.propertyIsEnumerable("toString"), f = "toString,toLocaleString,valueOf,hasOwnProperty,isPrototypeOf,propertyIsEnumerable,constructor".split(","), 
            j = f.length;
            return function(n) {
...
sigma.classes.EventDispatcher = function() {
    ...
    this.bind = function(f, j) {
        ...
        ("string" == typeof f ? f.split(" ") : f).forEach(function(f) {
            i[f] || (i[f] = []);
			if(i[f].push) { //payola mod i[f].push -> ...
				i[f].push({h: j,one: !1})
			} else {
				i[f].append({h: j,one: !1})
			}
        });
        return m
    };
    ...
    this.dispatch = function(f, j) {
        i[f] && (i[f].foreach(function(i) { //payola mod i[f].forEach -> i[f].foreach
            i.h({type: f,content: j,target: m})
...
(function() {
    ...
    function m() {
        function b(a) {
            return {x: a.x,y: a.y,size: a.size,degree: a.degree,inDegree: a.inDegree,outDegree: a.outDegree,displayX: a.displayX,displayY: a.displayY,displaySize: a.displaySize,label: a.label,id: a.id,color: a.color,fixed: a.fixed,active: a.active,hidden: a.hidden,forceLabel: a.forceLabel,attr: a.attr}
        }
        function l(a) {
        ...
        this.addEdge = function(a, b, c, g) {
            ...
            if (!d.nodesIndex[c])
                throw Error("Edge's target \"" + 
                c + '" does not exist yet.');
            g = g || {};
            b = {source: d.nodesIndex[b],target: d.nodesIndex[c],size: 1,weight: 1,displaySize: 0.5,label: a.toString(),id: a.toString(),hidden: !1,attr: {}};
            b.source.degree++;
            b.source.outDegree++;
            b.target.degree++;
            b.target.inDegree++;
            ...
            ((a instanceof Array ? a : [a]) || []).forEach(function(a) {
                if (d.nodesIndex[a]) {
                    ...
                    d.edges = d.edges.filter(function(c) {
                        if (c.source.id == a)
                            return delete d.edgesIndex[c.id], c.target.degree--, c.target.inDegree--, !1;
        ...
        this.iterEdges = function(a, b) {
            ...
            d.edges, g = c;//.map(l); //payola mod  g = c.map(l) -> g = c
            g.forEach(a); 
        ...
        this.iterNodes = function(a, g) {
            ...
            }) : d.nodes, l = c;//.map(b); payola mod l=c.map(b) -> l=c
            l.forEach(a);
        ...
        this.rescale = function(a, b, c, g) {
            ...
            Math.max(a / Math.max(k - l, 1), b / Math.max(p - s, 1)) : Math.min(a / Math.max(k - l, 1), b / Math.max(p - s, 1)), i = (d.p.maxNodeSize || h) / f;
            k += i;
            l -= i;
            p += i;
            s -= i;
            var f = "outside" == d.p.scalingMode ? Math.max(a / Math.max(k - l, 1), b / Math.max(p - s, 1)) : Math.min(a / Math.max(k - l, 1), b / Math.max(p - s, 1)), j, n;
            ...
            c && d.nodes.forEach(function(c) {
                ...
                if (!c.fixed)
                    c.displayX = (c.x - (k + l) / 2) * f + a / 2, c.displayY = (c.y - (p + s) / 2) * f + b / 2
            });
            g && d.edges.forEach(function(a) {
        ...
        this.translate = function(a, b, c, g, l) {
            ...
            g && d.nodes.forEach(function(d) {
                if (!d.fixed)
                    d.displayX = d.displayX * c + a, d.displayY = d.displayY * c + b;
                d.displaySize *= h
            });
        ...
        this.checkHover = function(a, b) {
            ...
            d.nodes.forEach(function(d) {
                if (d.hidden)
                    ...
                    l = d.displaySize;
                    var s = d.hover, p = c < l && g < l && Math.sqrt(c * c + g * g) < l;
                    if (s && !p)
                        d.hover = !1, f.push(d.id);
                    else if (p && !s)
                        d.hover = !0, h.push(d.id)
                }
            });
    ...
    function f(b, l) {
        function g() {
            var a;
            a = "<p>GLOBAL :</p>";
            for (var b in d.p.globalProbes)
                a += "<p>" + b + " : " + 
                d.p.globalProbes[b]() + "</p>";
            a += "<br><p>LOCAL :</p>";
            for (b in d.p.localProbes)
                a += "<p>" + b + " : " + d.p.localProbes[b]() + "</p>";
            d.p.dom.innerHTML = a;
            return d
        }
        sigma.classes.Cascade.call(this);
        ...
        this.instance = b;
        this.monitoring = !1;
        this.p = {fps: 40,dom: l,globalProbes: {"Time (ms)": sigma.chronos.getExecutionTime,Queue: sigma.chronos.getQueuedTasksCount,Tasks: sigma.chronos.getTasksCount,FPS: sigma.chronos.getFPS},localProbes: {"Nodes count": function() {
        ...
        this.activate = function() {
            if (!d.monitoring)
                d.monitoring = window.setInterval(g, 1E3 / d.p.fps);
            return d
        };
        this.desactivate = function() {
            if (d.monitoring)
                window.clearInterval(d.monitoring), d.monitoring = null, d.p.dom.innerHTML = "";
            return d
        }
    }
    function j(b) {
        function l(b) {
            if (a.p.mouseEnabled && (g(a.mouseX, a.mouseY, a.ratio * (0 < (void 0 != b.wheelDelta && b.wheelDelta || void 0 != b.detail && -b.detail) ? a.p.zoomMultiply : 1 / a.p.zoomMultiply)), a.p.blockScroll))
                b.preventDefault ? b.preventDefault() : b.returnValue = !1
        }
        function g(b, c, g) {
            if (!a.isMouseDown && 
            (window.clearInterval(a.interpolationID), w = void 0 != g, i = a.stageX, j = b, o = a.stageY, k = c, h = g || a.ratio, h = Math.min(Math.max(h, a.p.minRatio), a.p.maxRatio), m = a.p.directZooming ? 1 - (w ? a.p.zoomDelta : a.p.dragDelta) : 0, a.ratio != h || a.stageX != j || a.stageY != k))
                d(), a.interpolationID = window.setInterval(d, 50), a.dispatch("startinterpolate")
        }
        function d() {
            m += w ? a.p.zoomDelta : a.p.dragDelta;
            m = Math.min(m, 1);
            var b = sigma.easing.quadratic.easeout(m), c = a.ratio;
            a.ratio = c * (1 - b) + h * b;
            ...
            if (1 <= m)
                window.clearInterval(a.interpolationID), b = a.ratio, w ? (a.ratio = h, a.stageX = j + (a.stageX - j) * a.ratio / b, a.stageY = k + (a.stageY - k) * a.ratio / b) : (a.stageX = j, a.stageY = k), a.dispatch("stopinterpolate")
        }
        sigma.classes.Cascade.call(this);
        sigma.classes.EventDispatcher.call(this);
        var a = this;
        ...
        var f = 0, c = 0, i = 0, o = 0, h = 1, j = 0, k = 0, s = 0, p = 0, C = 0, n = 0, m = 0, w = !1;
        ...
        b.addEventListener("mousemove", function(b) {
            ...
            if (a.isMouseDown) {
                var d = a.mouseX - f + i, h = a.mouseY - 
                c + o;
                if (d != a.stageX || h != a.stageY)
                    p = s, n = C, s = d, C = h, a.stageX = d, a.stageY = h, a.dispatch("drag")
            }
            a.dispatch("move");
            b.preventDefault ? b.preventDefault() : b.returnValue = !1
        }, !0);
        b.addEventListener("mousedown", function(b) {
            if (a.p.mouseEnabled)
                a.isMouseDown = !0, a.dispatch("mousedown"), i = a.stageX, o = a.stageY, f = a.mouseX, c = a.mouseY, p = s = a.stageX, n = C = a.stageY, a.dispatch("startdrag"), b.preventDefault ? b.preventDefault() : b.returnValue = !1
        }, !0);
        document.addEventListener("mouseup", function(b) {
            if (a.p.mouseEnabled && a.isMouseDown)
                a.isMouseDown = 
                !1, a.dispatch("mouseup"), (i != a.stageX || o != a.stageY) && g(a.stageX + a.p.inertia * (a.stageX - p), a.stageY + a.p.inertia * (a.stageY - n)), b.preventDefault ? b.preventDefault() : b.returnValue = !1
        }, !0);
    ...
    function n(b, l, g, d, a, f, c) {
        function i(a) {
            var b = d, c = "fixed" == h.p.labelSize ? h.p.defaultLabelSize : h.p.labelSizeRatio * a.displaySize;
            ...
            if (h.p.labelHoverShadow)
                b.shadowOffsetX = 0, b.shadowOffsetY = 0, b.shadowBlur = 4, b.shadowColor = h.p.labelHoverShadowColor;
        ...
        sigma.classes.Cascade.call(this);
        var h = this;
        this.p = {labelColor: "default",defaultLabelColor: "#000",labelHoverBGColor: "default",defaultHoverLabelBGColor: "#fff",labelHoverShadow: !0,labelHoverShadowColor: "#000",
            ...
            defaultNodeHoverColor: "#fff",nodeActiveColor: "node",defaultNodeActiveColor: "#fff",borderSize: 0,nodeBorderColor: "node",defaultNodeBorderColor: "#fff",edgesSpeed: 200,nodesSpeed: 200,labelsSpeed: 200};
        var j = f, k = c;
        ...
        this.task_drawLabel = function() {
            for (var b = a.nodes.length, c = 0; c++ < h.p.labelsSpeed && h.currentLabelIndex < b; )
                if (h.isOnScreen(a.nodes[h.currentLabelIndex])) {
                    var d = a.nodes[h.currentLabelIndex++], l = g;
        ...
        this.task_drawEdge = function() {
            for (var b = a.edges.length, c, d, g = 0; g++ < h.p.edgesSpeed && h.currentEdgeIndex < b; )
                ...
                c = e.source, d = e.target, e.hidden || c.hidden || d.hidden || !h.isOnScreen(c) && !h.isOnScreen(d))
                    h.currentEdgeIndex++;
                else {
                    c = a.edges[h.currentEdgeIndex++];
                    d = c.source.displayX;
                    var f = c.source.displayY, o = c.target.displayX, i = c.target.displayY, j = c.color;
                    if (!j)
        ...
        this.task_drawNode = function() {
            for (var c = a.nodes.length, d = 0; d++ < h.p.nodesSpeed && h.currentNodeIndex < c; )
                if (h.isOnScreen(a.nodes[h.currentNodeIndex])) {
                    var g = a.nodes[h.currentNodeIndex++], l = Math.round(10 * g.displaySize) / 10, f = b;
                    f.fillStyle = g.color;
                    f.beginPath();
    ...
    function u(b, l) {
        ...
        this.draw = function(a, b, d, l) {
            ...
            for (var f in c.domElements)
                "canvas" == c.domElements[f].nodeName.toLowerCase() && (void 0 == l[f] || 0 <= l[f]) && c.domElements[f].getContext("2d").clearRect(0, 0, c.domElements[f].width, c.domElements[f].height);
            ...
            if (a)
                if (1 < a)
                    ...
                    sigma.chronos.addTask(c.plotter.task_drawNode, "node_" + c.id, !1), l = !0, f = "node_" + 
                    c.id;
            if (d)
                if (1 < d)
                    ...
                    f ? sigma.chronos.queueTask(c.plotter.task_drawLabel, "label_" + c.id, f) : sigma.chronos.addTask(c.plotter.task_drawLabel, "label_" + c.id, !1), l = !0, f = "label_" + c.id;
            if (b)
                if (1 <= b) //Payola mod "1 < b" -> "1 <= b"
                    for (; c.plotter.task_drawEdge(); )
                        ;
                else
                    f ? sigma.chronos.queueTask(c.plotter.task_drawEdge, "edge_" + c.id, f) : sigma.chronos.addTask(c.plotter.task_drawEdge, "edge_" + c.id, !1), l = !0, f = "edge_" + c.id;
            c.dispatch("draw");
            c.refresh();
        ...
        b) {
            ...
            if (d != c.width || g != c.height) {
                for (var f in c.domElements)
                    c.domElements[f].setAttribute("width", c.width + "px"), c.domElements[f].setAttribute("height", c.height + "px");
    ...
    sigma.chronos = new function() {
        ...
        function g() {
            ...
            if (!t[z].task()) {
                ...
                v = v.filter(function(b) {
                    b.taskParent == 
                    a && t.push({taskName: b.taskName,task: b.task});
        ...
        function i(a, b, c) {
            ...
            t.$push({taskName: b,task: a}); //Payola modification t.push -> t.$push
            m = !(!m && !(c && d() || 1));
            return h
        }
        function c(a) {
        ...
        function o(a) {
            if (void 0 != q[a.content.taskName])
                q[a.content.taskName].del || !q[a.content.taskName].condition() ? delete q[a.content.taskName] : q[a.content.taskName].on = !1, 0 == c(!0) && j()
        }
        sigma.classes.EventDispatcher.call(this);
        var h = this, m = !1, k = 80, n = 0, p = 0, r = 1E3 / k, u = r, y = 
        0, w = 0, x = 0, D = 0, q = {}, t = [], v = [], z = 0;
        ...
        this.queueTask = function(a, b, c) {
            ...
            v.push({taskParent: c,taskName: b,task: a});
            return h
        };
    ...
    sigma.tools.drawRoundRect = function(b, f, g, d, a, i, c) {
        var i = i ? i : 0, j = c ? c : [], j = "string" == typeof j ? j.split(" ") : j, c = i && (0 <= j.indexOf("topleft") || 0 <= j.indexOf("top") || 0 <= j.indexOf("left")), m = i && (0 <= j.indexOf("topright") || 0 <= j.indexOf("top") || 0 <= j.indexOf("right")), 
        h = i && (0 <= j.indexOf("bottomleft") || 0 <= j.indexOf("bottom") || 0 <= j.indexOf("left")), j = i && (0 <= j.indexOf("bottomright") || 0 <= j.indexOf("bottom") || 0 <= j.indexOf("right"));
        b.moveTo(f, g + i);
    ...
    sigma.tools.getRGB = function(b, f) {
        var b = b.toString(), g = {r: 0,g: 0,b: 0};
        if (3 <= 
        b.length && "#" == b.charAt(0)) {
            var d = b.length - 1;
            6 == d ? g = {r: parseInt(b.charAt(1) + b.charAt(2), 16),g: parseInt(b.charAt(3) + b.charAt(4), 16),b: parseInt(b.charAt(5) + b.charAt(5), 16)} : 3 == d && (g = {r: parseInt(b.charAt(1) + b.charAt(1), 16),g: parseInt(b.charAt(2) + b.charAt(2), 16),b: parseInt(b.charAt(3) + b.charAt(3), 16)})
        }
        f && (g = [g.r, g.g, g.b]);
        return g
    };
